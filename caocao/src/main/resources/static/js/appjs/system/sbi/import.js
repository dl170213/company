...
layui.use('upload', function () {
    ...
    var uploadInst = upload.render({
        elem: '#upload', //绑定元素
        url: '/system/sbi/importExcel', //上传接口
        size: 10240,//KB
        accept: 'file',
        exts:"xls|xlsx",
        multiple: true,
     ...
     //   bindAction:"#upload",
        before: function(obj){ //obj参数包含的信息，跟 choose回调完全一致，可参见上文。
            ...
            //    layer.msg(r.failList[0].errorMsg);
                ...
                $.each(r.infos,function (key,value) {//打印错误信息
                    $("#error_msg").text($("#error_msg").text()+"\n文件："+value.name+" 操作结果：");
                    if(value.info.code==0||value.info.code==1||value.info.code==3){
                        $("#error_msg").text($("#error_msg").text()+"\n"+value.info.msg);
                    }else{
                        $.each(value.info.failList,function (key,value) {//打印错误信息
                            $("#error_msg").text($("#error_msg").text()+"\n第 "+(value.rowNum+1)+" 行："+value.errorMsg);
                        });
                    }
                    $("#error_msg").text($("#error_msg").text()+"\n");
